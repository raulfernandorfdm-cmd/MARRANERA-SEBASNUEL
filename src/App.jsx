...
export default function App() {
  ...
  const exportarPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Marranera Sebasnuel", 10, 15);

    doc.setFontSize(12);
    doc.text(`Cliente: ${cliente}`, 10, 30);
    doc.text(`Teléfono: ${telefono}`, 10, 38);
    doc.text(`Vendedor: ${usuarioActual}`, 10, 46);
    doc.text(`Método de pago: ${metodoPago}`, 10, 54);
      ...
      {productos.map((p, i) => (
        ...
        >
          ...
          <input
            ...
            value={p.kilos}
            onChange={(e) =>
              actualizarProducto(i, "kilos", e.target.value)
            }
          />
          <input
            type="number"
            value={p.precio}
            onChange={(e) =>
              actualizarProducto(i, "precio", e.target.value)
            }
          />
        </div>
      ))}
